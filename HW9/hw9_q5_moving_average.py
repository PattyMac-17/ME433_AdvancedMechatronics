...
if __name__ == '__main__':
    ...
    for name in ('sigA', 'sigB', 'sigC', 'sigD'):
        ...
        cand_path = os.path.join(out_dir, 'hw9_q5_{}_maf_candidates.png'.format(name))
        ...
        final_path = os.path.join(out_dir, 'hw9_q5_{}_maf_final.png'.format(name))
        plot_time_compare(
            t, x, y,
            '{} MAF -- X = {} averaged points (Fs = {:.1f} Hz)'.format(name, k_best, Fs),
            final_path,
        )
        ...
        summary.append((name, Fs, k_best, [os.path.basename(p) for p in (cand_path, final_path, fft_path)]))
