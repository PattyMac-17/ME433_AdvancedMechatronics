...
#define NSAMPLES 400
volatile signed short desired_log[NSAMPLES];
volatile signed short actual_log[NSAMPLES];
float Kp = 0.8f;
float Ki = 0.08f;
  ...
  while (1)
  {
    ...
    if (HAL_UART_Receive(&hcom_uart[COM1], &ch, 1, 10) == HAL_OK)
    {
        if (ch == 'a')
        {
            ...
            {
                printf("%d %d %d\r\n", i, desired_log[i], actual_log[i]);
            }
        }
    }

    /* CAN demo code disabled for HW16 ADC test:
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if (htim == &htim2)
    {
        ...
        if (state == 1)
        {
            ...
            static float desired = 200.0f;

            signed short actual = readINA219(INA219_REG_CURRENT);
            float error = desired - (float)actual;
            eint += error;
            float u = Kp * error + Ki * eint;

            if (u >  2400.0f) u =  2400.0f;
            if (u < -2400.0f) u = -2400.0f;
            ...
            {
                __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, 2400);
                __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, 2400 - (uint16_t)u);
            }
            else
            {
                __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, 2400);
                __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, 2400 - (uint16_t)(-u));
            }

            
            desired_log[counter] = (signed short)desired;
            actual_log[counter]  = actual;

            counter++;
            if (counter % 100 == 0) desired = -desired;   
            if (counter >= NSAMPLES)                        
            {
                __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, 2400);   
                __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, 2400);
